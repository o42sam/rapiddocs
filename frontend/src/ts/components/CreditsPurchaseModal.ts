...
export class CreditsPurchaseModal {
  ...
  private createModal(): HTMLElement {
    ...
    modal.innerHTML = `
      <div class="relative top-10 mx-auto p-5 max-w-4xl mb-10">
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
          <!-- Header -->
          <div class="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-6 md:px-8 md:py-8">
            <div class="flex justify-between items-center">
              <div>
                <h2 class="text-2xl md:text-3xl font-bold text-white">Buy Credits</h2>
                <p class="text-blue-100 mt-1">Pay with Bitcoin to continue creating documents</p>
              </div>
              <button id="close-credits-modal" class="text-white hover:text-gray-200 transition-colors">
          ...
          <div class="px-6 py-8 md:px-8">
            ...
            <div id="bitcoin-payment-section" class="hidden">
              <div class="border-t border-gray-200 pt-6">
                <div class="flex items-center justify-between mb-6">
                  <h3 class="text-xl font-bold text-gray-900">Bitcoin Payment</h3>
                  <button id="back-to-packages-btn" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
                    ← Back to Packages
                  </button>
                </div>
                ...
                <div id="payment-loading" class="text-center py-8">
                  ...
                  <p class="mt-4 text-gray-600">Generating payment address...</p>
                </div>

                <!-- Payment Details -->
                <div id="payment-details" class="hidden">
                  <div class="bg-gradient-to-br from-blue-50 to-purple-50 border border-blue-200 rounded-lg p-6 mb-6">
                    <div class="grid md:grid-cols-2 gap-6">
                      <!-- Payment Info -->
                      <div>
                        <div class="mb-4">
                          <label class="block text-sm font-medium text-gray-700 mb-2">Package:</label>
                          <p id="payment-package-info" class="text-lg font-semibold text-gray-900"></p>
                        </div>

                        <div class="mb-4">
                          <label class="block text-sm font-medium text-gray-700 mb-2">Amount to Pay:</label>
                          <div class="flex items-baseline gap-2">
                            <p id="payment-btc-amount" class="text-2xl font-bold text-orange-600"></p>
                            <p id="payment-usd-amount" class="text-sm text-gray-600"></p>
                          </div>
                        </div>
                        ...
                        <div class="mb-4">
                          ...
                          <div id="payment-countdown" class="flex items-center justify-center gap-2 bg-gradient-to-r from-orange-100 to-red-100 border-2 border-orange-300 rounded-lg p-3">
                            <svg class="w-5 h-5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <span id="countdown-timer" class="text-2xl font-bold text-orange-700 font-mono">59:59</span>
                          </div>
                        </div>
                      ...
                      <div class="flex flex-col items-center justify-center">
                        <p class="text-sm font-medium text-gray-700 mb-3">Scan QR Code:</p>
                        <div class="bg-white p-4 rounded-lg border-2 border-gray-200">
                          <img id="payment-qr-code" src="" alt="Bitcoin QR Code" class="w-48 h-48" />
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <!-- Payment Status -->
                  <div id="payment-status" class="bg-white border-2 border-gray-200 rounded-lg p-6">
                    <div class="flex items-center justify-between mb-4">
                      <h4 class="text-lg font-semibold text-gray-900">Payment Status</h4>
                      <span id="status-badge" class="px-3 py-1 bg-yellow-100 text-yellow-800 text-sm font-medium rounded-full">
                        Waiting for payment
                      </span>
                    </div>

                    <div id="status-message" class="text-gray-600 mb-4">
                    ...
                    <div id="confirmation-progress" class="hidden">
                      ...
                      <div class="w-full bg-gray-200 rounded-full h-3">
                        <div id="confirmations-bar" class="bg-blue-600 h-3 rounded-full transition-all duration-500" style="width: 0%"></div>
                      </div>
                    </div>

                    <!-- Transaction Hash -->
                    <div id="tx-hash-section" class="hidden mt-4 text-sm">
                      <span class="text-gray-600">Transaction: </span>
                      <a id="tx-hash-link" href="#" target="_blank" class="text-blue-600 hover:underline font-mono"></a>
                    </div>
                  </div>
                </div>

                <!-- Error Message -->
                <div id="payment-error" class="hidden bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm mt-4"></div>
              </div>
            </div>
            ...
            <div id="success-section" class="hidden">
              <div class="text-center py-8">
                ...
                <h3 class="text-2xl font-bold text-gray-900 mb-2">Credits Purchased!</h3>
                <p class="text-gray-600 mb-4" id="success-message"></p>
                <button
  ...
  private renderPackages(): string {
    ...
    return this.packages.map((pkg) => `
      <div class="package-card relative border-2 border-gray-200 rounded-xl p-6 hover:border-blue-500 hover:shadow-lg transition-all cursor-pointer ${pkg.id === 'medium' ? 'border-blue-500 shadow-lg' : ''}" data-package-id="${pkg.id}">
        ${pkg.id === 'medium' ? '<div class="absolute top-0 right-0 bg-blue-500 text-white text-xs font-bold px-3 py-1 rounded-bl-lg rounded-tr-lg">POPULAR</div>' : ''}

        ${packageIcons[pkg.id as keyof typeof packageIcons] || ''}

        <h3 class="text-xl font-bold text-gray-900 text-center mb-2">${pkg.name}</h3>
        ...
        <div class="text-center mb-4">
          ...
          <p class="text-3xl font-bold text-gray-900">${pkg.credits.toLocaleString()}</p>
          <p class="text-sm text-gray-500">Credits</p>
        </div>

        <div class="text-center mb-4">
          <p class="text-2xl font-bold text-blue-600">$${pkg.price.toFixed(2)}</p>
          <p class="text-xs text-gray-500 mt-1">${(pkg.price / pkg.credits * 1000).toFixed(2)}¢ per 1000 credits</p>
        </div>

        <button class="w-full btn-primary select-package-btn" data-package-id="${pkg.id}">
          Pay with Bitcoin
        </button>
      </div>
    `).join('');
